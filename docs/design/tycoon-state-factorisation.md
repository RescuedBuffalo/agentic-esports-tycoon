# Tycoon-layer state factorisation (companion doc)

Status: **Planning / draft**. **Companion** to BUF-81 — the management-layer
state. The neural world-model decision that BUF-81 actually closes lives in
[`world-model-state.md`](world-model-state.md). Pairs with
[`tycoon-obs-action-space.md`](tycoon-obs-action-space.md).
