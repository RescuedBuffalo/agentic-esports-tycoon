# Tycoon-layer observation / action space (companion doc)

Status: **Planning / draft**. **Companion** to BUF-79 — the management-layer
contract. The match-engine RL contract that BUF-79 actually closes lives in
[`observation-action-space.md`](observation-action-space.md). Pairs with
[`tycoon-state-factorisation.md`](tycoon-state-factorisation.md).

We need a single typed contract that every agent backend (scripted, RL, LLM) can
satisfy. This document fixes that contract for the first vertical slice of the
**tycoon** layer. Anything flagged _provisional_ here will need a follow-up
...
the agent-binding config (planned).
...
`src/esports_sim/obs/encode.py` (planned). LLM agents skip the tensor view entirely
