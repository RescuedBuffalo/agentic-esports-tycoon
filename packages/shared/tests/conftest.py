...
# Sibling-import path: pytest's ``--import-mode=importlib`` (configured at
# the workspace root so two ``tests/`` directories don't collide on a
# shared module name) skips the sys.path manipulation that the default
# ``prepend`` mode would otherwise do — adding it here keeps
# ``from fixtures import ...`` working in the per-test files.
